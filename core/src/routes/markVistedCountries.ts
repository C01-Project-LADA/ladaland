...
router.post('/markVisitedCountries', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!countryCode || !['add', 'remove'].includes(action)) {
    res.status(400).json({ error: 'Invalid request. Provide a countryCode and action (add or remove).' });
    return;
  }
  ...
  try {
    ...
    let visitedCountries = user.visitedCountries ? user.visitedCountries.split(',') : [];

    if (action === 'add' && !visitedCountries.includes(countryCode)) {
      visitedCountries.push(countryCode);
    } else if (action === 'remove' && visitedCountries.includes(countryCode)) {
      visitedCountries = visitedCountries.filter((country) => country !== countryCode);
    }
